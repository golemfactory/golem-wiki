...
A typical gWASM application consists of a client and WASM binary. The client is non-WASM software. Its purpose is to connect to a Golem node, create tasks and process results. An example of a client is [g-flite](Products/Brass-Beta/gWASM?id=sample-application-g-flite). The WASM binary consists of the actual binary (.wasm) and its JavaScript glue code (.js file). Both are generated automatically when compiling with Emscripten. When computing a task, the binary (along with its input) is transferred to the provider's machine and executed by the WASM engine. An example of this is g-flite compiled to WebAssembly.
